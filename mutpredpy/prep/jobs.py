"""
Jobs Module for MutPredPy

This module handles the splitting and management of job data for MutPred2,
including dividing large mutation datasets into smaller, processable chunks
based on time and memory constraints.
"""
...
def split_mutations(x, threshold, num_overflow_mutations, col_mapping):
    ...
    size_of_splits = int(
        len(leftover_mutations)
        / (
            (
                x["Time Estimate (hrs)"]
                - (num_overflow_mutations * x["Time per Mutation (hrs)"])
            )
            / threshold
        )
    )
    ...
    if len(overflow_mutations) > 0:
        return [" ".join(overflow_mutations)] + [
...
def split_sequence(data, cur_number, threshold, col_mapping):
    ...
    data[mutation_column] = data.apply(
        lambda x: split_mutations(x, threshold, num_overflow_mutations, col_mapping),
        axis=1,
    )
...
def split_data(prepare, data, col_mapping, file_number=1):
    ...
    def write_sequence(
        row, file_number, time_estimate, location
    ):  # pylint: disable=W0613
        """Writes a sequence to a file with a given file number and time estimate."""
        header = f">{'|'.join(list(row[col_mapping['id_column']]))} \
            {row[col_mapping['mutation_column']]}\n"
    ...
    job_information["Normal Memory"] = job_information["Memory Minimum"].apply(
        lambda x: x <= 5000
    )
    ...
    job_information["Time Estimate"] = job_information["Time Estimate"].apply(
        lambda x: round(x, 2)
    )
    job_information["Memory Minimum"] = job_information["Memory Minimum"].apply(
        lambda x: round(x, 2)
    )
